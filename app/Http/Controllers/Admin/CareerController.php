...
{
    protected $interface;
    public function __construct(CareerInterface $interface)
    {
        $this->interface= $interface;
        $this->middleware(['permission:Career view'])->only(['index']);
        $this->middleware(['permission:Career create'])->only(['create']);
        $this->middleware(['permission:Career edit'])->only(['edit']);
        $this->middleware(['permission:Career destroy'])->only(['destroy']);
        $this->middleware(['permission:Career status'])->only(['status']);
        $this->middleware(['permission:Career restore'])->only(['restore']);
