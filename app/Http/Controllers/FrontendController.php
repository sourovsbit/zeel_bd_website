...
class FrontendController extends Controller
{
    ...
    {
        $data = ProductInformation::where('item_id',3)->count();

        $products = ProductInformation::where('item_id',3)->where('status',1)->get();
        $total_products = ProductInformation::where('item_id',3)->where('status',1)->count();

        return view("frontend.shop",compact('data','products','total_products'));
    }
    ...
    {
        $data = ProductInformation::where('id',$id)->first();

        $color = ProductColorInfo::where('product_id',$id)->get();

        $image = ProductImage::where('product_id',$data->product_id)->skip(1)->take(4)->get();

        $activeImage = ProductImage::where('product_id',$data->product_id)->first();

        return view('frontend.sell_page',compact('data','id','color','image','activeImage'));
    }
