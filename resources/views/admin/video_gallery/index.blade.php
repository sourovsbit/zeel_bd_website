...
<div class="content">
    ...
    <!-- Active Link -->
    @slot('active_link')
    @lang('video_gallery.index_title')
    @endslot
    ...
    @if(Auth::user()->can('Video Gallery trash'))
    <!-- button two -->
    ...
    {{route('video_gallery.trash_list')}}
    @endslot
            ...
            <div class="table-responsive">
                <table class="table myTable  fs--1 mb-0">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>@lang('video_gallery.title')</th>
                            <th>@lang('video_gallery.url')</th>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    $(".myTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('video_gallery.index') }}",
        columns: [
            {data: 'sl', name: 'sl'},
...
<script>
    function changeVideoGalleryStatus(id)
    {
        // alert(id);
        $.ajax({
            headers : {
