...
@section('content')
    ...
    <section class="py-5" style="background:#f8fafc; min-height:60vh; margin-top:100px;">
        <div class="container">
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
                <div>
                    <p class="text-uppercase small mb-1" style="letter-spacing:2px; color:#d88a1f; font-weight:700;">Category Products</p>
                    <h2 class="fw-bold mb-0" style="color:#1f2937;">{{ $category['name'] ?? 'Category' }}</h2>
                </div>
                    ...
                    @foreach ($products as $product)
                        ...
                        <div class="col-xl-3 col-lg-4 col-md-6">
                            <div class="card h-100 border-0 rounded-4 overflow-hidden"
                                style="box-shadow:0 10px 22px rgba(0,0,0,0.06);">
                                <img src="{{ $imagePath }}" alt="{{ $productName }}" class="w-100"
                                ...
                                <div class="card-footer bg-white border-0 pt-0 pb-3 px-3">
                                    <a href="{{ $productId ? url('sell_page/' . $productId) : '#' }}" class="btn btn-sm w-100 rounded-pill"
                                        style="background:#e9eff8; color:#1f2937;">View Details</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <div class="bg-white border rounded-4 py-5 text-center">
                    <h5 class="mb-2 text-secondary">No products found for this category.</h5>
